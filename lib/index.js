...
function createClient(config) {
  ...
  return new import_coc2.LanguageClient("clojure", "Clojure Language Client", serverOptions, clientOptions);
}
...
async function titleForBinding(title) {
  const result = await import_coc3.window.requestInput(title, "");
...
function createCommands(context, client) {
  context.subscriptions.push(import_coc3.commands.registerCommand("lsp-clojure-server-info", async () => {
    return client.sendRequest("clojure/serverInfo/log");
  }), import_coc3.commands.registerCommand("lsp-clojure-cursor-info", async () => {
    return client.sendRequest("workspace/executeCommand", {
      command: "cursor-info",
