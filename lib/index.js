...
async function requestProjectTree(client, param) {
  const result = await client.sendRequest("clojure/workspace/projectTree/nodes", param);
...
var ProjectTree = class {
  ...
  async resolveTreeItem(item, node) {
    ...
    logger.debug("resolveTreeItem result", JSON.stringify([updatedNode, newItem], null, 4));
    return newItem;
  }
...
async function projectTree(client, context) {
  ...
  context.subscriptions.push(import_coc3.commands.registerCommand("_project-tree", (node) => {
    return adapter.resolveTreeItem(void 0, node);
  }));
  await treeView.show();
}
...
var clojureCommands = (() => {
  const { commands: commands4 } = commands_exports;
  const mergedCommands = /* @__PURE__ */ new Map();
  commands4.concat(complexCommands).forEach((cmd) => {
...
function registerKeymap(context, cmd, keymaps) {
  ...
  try {
    ...
    import_coc4.workspace.nvim.command(`call nvim_set_keymap('n', '${keymap}', ${rhs}, ${opts})`, true);
    context.subscriptions.push(import_coc4.Disposable.create(() => {
...
async function getLatestVersion() {
  try {
    const latestReleaseRaw = await fetchFromUrl("https://api.github.com/repos/clojure-lsp/clojure-lsp/releases/latest");
...
async function maybeDownloadClojureLsp(extensionPath, msg) {
  ...
  if (currentVersion !== downloadVersion && downloadVersion !== "") {
    const choice = await import_coc5.window.showQuickpick(["Yes", "No"], `clojure-lsp is ${msg}. Download from Github?`);
    if (choice == 0) {
...
async function activate(context) {
  ...
  context.subscriptions.push(import_coc6.services.registLanguageClient(client));
  context.subscriptions.push(import_coc6.languages.registerSignatureHelpProvider(documentSelector, new ClojureSignatureHelpProvider(client), ["(", " "]));
